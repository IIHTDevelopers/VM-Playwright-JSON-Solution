  ...
  async performRadiologyRequestAndHandleAlert() {
    try {
      ...
      for (let i = 0; i < 2; i++) {
        ...
        await PatientSearchHelper.highlightElement(this.radiologyModule);
        await this.radiologyModule.click();
        await PatientSearchHelper.highlightElement(this.listRequestSubModule);
        ...
        await PatientSearchHelper.highlightElement(this.addReportButton);
        await this.addReportButton.click();

        // Step 5: Close the modal
        await PatientSearchHelper.highlightElement(this.closeModalButton);
