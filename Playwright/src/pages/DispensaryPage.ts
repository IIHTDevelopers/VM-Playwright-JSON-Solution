...
export default class DispensaryPage {
  ...
  async verifyActiveCounterMessageInDispensary() {
    try {
      ...
      await PatientSearchHelper.highlightElement(this.dispensary.dispensaryLink);
      await this.dispensary.dispensaryLink.click();
      await PatientSearchHelper.highlightElement(this.dispensary.activateCounter);
      ...
      if (counterCount >= 1) {
        ...
        await PatientSearchHelper.highlightElement(randomCounter);
  ...
  async generateMorningCounterReport() {
    ...
    await PatientSearchHelper.highlightElement(this.dispensary.reports);
    await this.dispensary.reports.click();

    await PatientSearchHelper.highlightElement(this.dispensary.userCollectionReport);
    await this.dispensary.userCollectionReport.click();

    await PatientSearchHelper.highlightElement(this.dispensary.fromDate);
    ...
    await PatientSearchHelper.highlightElement(this.dispensary.showReportButton);
  ...
  async searchPatientInDispensary(): Promise<boolean> {
    try {
      await PatientSearchHelper.highlightElement(this.dispensary.dispensaryLink);
      await this.dispensary.dispensaryLink.click();
      await PatientSearchHelper.highlightElement(this.dispensary.prescription);
