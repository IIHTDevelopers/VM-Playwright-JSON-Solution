...
export default class AppointmentPage {
  ...
  async verifypatientName() {
    try {
      ...
      if (counterCount > 0) {
        await PatientSearchHelper.highlightElement(
  ...
  async openNewVisitPageThroughKeyboardButton() {
    ...
    await PatientSearchHelper.highlightElement(this.appointment.appointmentLink);
    await this.appointment.appointmentLink.click();
    ...
    await PatientSearchHelper.highlightElement(this.appointment.newVisitTab);
